...
if (!messageListenerActive) {
    ...
    // Listen for the "PostMessage" from your Netlify Website
window.addEventListener("message", (event) => {
    // Only accept messages from your specific Netlify URL for security
    if (event.origin !== "https://reactov1.netlify.app") return;

    if (event.data.type === 'AUTO_REACTOR_RESPONSE') {
        console.log("Bridge received response request:", event.data.message);
...
async function sendMessageToChat(message) {
    // 1. Find the "Chat with everyone" button
    const chatButton = document.querySelector('[aria-label*="Chat with everyone"]');
    
    // 2. Check if the chat input exists. If not, click the button to open it.
    let chatInput = document.querySelector('textarea[name="chatTextInput"]') || 
                    document.querySelector('[data-placeholder="Send a message"]');

    if (!chatInput && chatButton) {
        chatButton.click();
        // Wait for the panel to slide open
        await new Promise(resolve => setTimeout(resolve, 800));
        chatInput = document.querySelector('textarea[name="chatTextInput"]') || 
                    document.querySelector('[data-placeholder="Send a message"]');
    }

    if (chatInput) {
        chatInput.focus();
        // Use execCommand to trick Google's React state into seeing the text
        document.execCommand('insertText', false, message);
        
        // Trigger the Send button
        setTimeout(() => {
            const sendBtn = document.querySelector('[aria-label="Send a message"]');
